...
{
public:
    explicit AI(Chessboard &board) : chessboard(board), rng(std::random_device{}()) {}
    ...
    std::vector<Types::Turn> generateAllLegalMoves(char player, int turn,
                                                   bool alt);
    float evaluateBoard();
    ...
    float quiescenceSearch(char player, float alpha, float beta,
                           int maxDepth = 3);
