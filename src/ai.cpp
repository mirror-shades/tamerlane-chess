...
float AI::minMaxHelper(GameLogic gameLogic, char player, int turn, bool alt, int depth, float alpha, float beta)
{
    ...
    const int maxMovesToConsider = std::min(static_cast<int>(allMoves.size()), 5);
    ...
    for (int i = 0; i < maxMovesToConsider; ++i)
    {
        ...
        float value = minMaxHelper(gameLogic, (player == 'w' ? 'b' : 'w'), turn + 1, alt, depth - 1, alpha, beta);
...
std::vector<Types::Turn> AI::generateAllLegalMoves(char player, int turn, bool alt)
{
    GameLogic gameLogic;
    ...
    for (int row = 0; row < Chessboard::rows; ++row)
    {
        for (int col = 0; col < Chessboard::cols; ++col)
        {
            ...
            if (piece[0] == player)
            {
                ...
                auto legalMoves = gameLogic.filterLegalMoves(possibleMoves, currentSquare, piece, player, alt);

                for (const auto &move : legalMoves)
                {
                    allMoves.emplace_back(Types::Turn{turn, player, currentSquare, move, piece, chessboard.getPiece(move), 0.0f});
                }
            }
        }
    }
...
float AI::evaluatePawnStructure(int col, int row, bool isWhite)
{
    ...
    {
        if (r != row && chessboard.getPiece({col, r}) == (isWhite ? "wp" : "bp"))
        {
            score -= 0.5f; // Penalty for doubled pawns
        }
    }

    // Check for isolated pawns
    bool isolated = true;
    for (int c = std::max(0, col - 1); c <= std::min(Chessboard::cols - 1, col + 1); ++c)
    {
        if (c != col)
        {
            for (int r = 0; r < Chessboard::rows; ++r)
    ...
    if (isolated)
        score -= 0.3f; // Penalty for isolated pawns
    ...
    {
        if (chessboard.getPiece({col - 1, row + direction}) == (isWhite ? "wp" : "bp"))
        {
            score += 0.2f; // Bonus for being part of a pawn chain
        }
    }
...
float AI::evaluatePieceMobility(const std::string &piece, int col, int row)
{
    ...
    Types::Coord currentSquare = {col, row};
    char player = piece[0];
    bool alt = false; // Assuming standard moves, not alternate moves
    ...
    {
    case 'R':                                       // Rook
        mobilityScore += legalMoves.size() * 0.05f; // Rooks benefit more from open lines
        break;
    case 'G':                                       // Giraffe
    case 'E':                                       // Elephant
        mobilityScore += legalMoves.size() * 0.03f; // Long-range pieces benefit from more options
        break;
    case 'M':                                       // Mongol (Knight)
        mobilityScore += legalMoves.size() * 0.04f; // Knights benefit from central positions
        break;
    case 'C':                                       // Camel (3x1 Knight)
        mobilityScore += legalMoves.size() * 0.03f; // Camels benefit from open board
        break;
    case 'T':                                        // Taliah (Restricted Bishop)
        mobilityScore += legalMoves.size() * 0.035f; // Taliahs benefit from open diagonals
        break;
    case 'W':                                       // War Machine
        mobilityScore += legalMoves.size() * 0.04f; // War Machines benefit from open lines
        break;
    case 'p': // Pawn
        // Pawns benefit more from forward moves
        for (const auto &move : legalMoves)
        {
            if ((player == 'w' && move.y > row) || (player == 'b' && move.y < row))
            {
                mobilityScore += 0.15f;
            }
        }
...
float AI::evaluateKingSafety(int col, int row, bool isWhite)
{
    ...
    for (int i = -1; i <= 1; ++i)
    {
        ...
        {
            std::string piece = chessboard.getPiece({col + i, row + pawnShieldDirection});
    ...
    for (int i = -1; i <= 1; ++i)
    {
        for (int j = -1; j <= 1; ++j)
        {
            if (i == 0 && j == 0)
                continue;
            if (col + i >= 0 && col + i < Chessboard::cols && row + j >= 0 && row + j < Chessboard::rows)
            {
...
float AI::evaluateCenterControl(int col, int row)
{
    ...
    if (col >= centerStartCol && col <= centerEndCol && row >= centerStartRow && row <= centerEndRow)
    {
        ...
        centerControlScore += 0.1f * (3 - distanceFromCenterCol) + 0.1f * (4 - distanceFromCenterRow);
...
float AI::quiescenceSearch(char player, float alpha, float beta, int maxDepth)
{
    ...
    for (const auto &move : captureMoves)
    {
        ...
        float score = -quiescenceSearch(player == 'w' ? 'b' : 'w', -beta, -alpha, maxDepth - 1);
...
std::vector<Types::Turn> AI::generateCaptureMoves(char player)
{
    ...
    for (int row = 0; row < Chessboard::rows; ++row)
    {
        for (int col = 0; col < Chessboard::cols; ++col)
        {
            ...
            if (piece[0] == player)
            {
                auto possibleMoves = gameLogic.getMoves(currentSquare, piece, player, false);
                auto legalMoves = gameLogic.filterLegalMoves(possibleMoves, currentSquare, piece, player, false);
